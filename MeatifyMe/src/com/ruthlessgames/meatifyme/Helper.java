...
public class Helper {
	...
	public Level get(Player pl,String filePath,boolean fromLocal,boolean campaign)
	{
		...
		try {
			...
			if(!fromLocal)
			a = xml_reader.parse(Gdx.files.internal("levels/" +filePath));
			else a = xml_reader.parse(Gdx.files.external(MeatifyMe.LEVELS_PATH + filePath));
	...
	public FileHandle[] listCustomLevels(){

			FileHandle dir = new FileHandle(Gdx.files.getExternalStoragePath()+MeatifyMe.LEVELS_PATH);
			Gdx.app.log("dir length", dir.list().length+"");
			return dir.list();
	}
	
	public FileHandle[] listLevels(){
